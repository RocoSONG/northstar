  ...
  "benefitValidation": [
    {
      ...
      "msg": "针对「权益本身」的资损/羊毛风险校验结论（成本失控、套利、重复领取等）"
    }
  ],
  "risks": [
    {
      "level": "low|medium|high",
      "content": "项目整体风险（获客成本、转化不及预期、渠道依赖、时间窗口、竞品、人群匹配等）"
    }
  ]
}
其中：
...
- benefitRules 基于用户选择的权益工具（优惠券/积分/会员权益等）与业务背景，设计 2-3 条具体权益规则。
- 三个板块各司其职、严禁内容重复：
  · benefitRules = 权益「怎么设计」（面额、门槛、叠加、限量）；
  · benefitValidation = 权益「安不安全」（成本失控、羊毛套利、重复领取等资损风险）；
  · risks = 项目「整体风险」（获客成本、转化、渠道依赖、时间窗口、竞品等），绝不要重复 benefitValidation 里已写过的资损内容。`;
